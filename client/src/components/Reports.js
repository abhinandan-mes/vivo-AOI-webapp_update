  ...
  const loadData = () => {
    ...
    Promise.all([
      ...
      .then(([checklistRes, checkpointRes, changeoverRes, laserRes, engineersRes]) => {
        ...
        // Note: setLaserChangeovers might not be defined depending on how the file was modified,
        // let's check if it exists in the surrounding code. I'll define a safe fallback.
        if (typeof setLaserChangeovers === 'function') {
  ...
  return (
    <table className="reports-table">
      ...
      <tbody>
        {rows.length === 0 ? (
          <tr>
            <td colSpan={isSuperAdmin ? 20 : 19} className="no-data-cell">
              {t('rep_no_data')}
            </td>
          </tr>
        ) : (
          rows.map((row) => (
            <tr key={row.id} className={selectedRows.includes(row.id) ? 'selected-row' : ''}>
              <td>
                <input type="checkbox" checked={selectedRows.includes(row.id)} onChange={(e) => onSelectRow(row.id, e.target.checked)} />
              </td>
              <td>{row.line || ''}</td>
              ...
              <td>
                <span className={`status-badge ${row.approval_status === 'APPROVED' ? 'status-approved' : row.approval_status === 'REJECTED' ? 'status-rejected' : 'status-pending'}`}>
                  {row.approval_status}
                </span>
              </td>
              
              <td>{renderBool(row.prog_name_check)}</td>
              <td>{renderBool(row.laser_param_check)}</td>
              <td>{renderBool(row.duplicate_code_check)}</td>
              <td>{renderBool(row.pcb_anti_reverse_check)}</td>
              <td>{renderBool(row.ab_barcode_check)}</td>
              <td>{renderBool(row.laser_sequence_check)}</td>
              <td>{renderBool(row.laser_position_check)}</td>
