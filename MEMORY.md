...
### Resolved: Client IP Logging and Dashboard Metric Fixes (September 2026)
* **IP Proxy Fix**: Modified \client/server.js\ to set \xfwd: true\ in \createProxyMiddleware\ so that the backend receives the actual client IP (\x-forwarded-for\) instead of the local server proxy IP (\::1\ or \10.172.9.196\).
* **Reports Summary Cards**: Added "Changeover Checksheet" and "Laser Changeover" metric cards to the top of the \Reports.js\ dashboard. 
* **Metrics Refinement**: Modified the \enderSummaryCard\ component to accept a \hideNotFilled\ flag, completely hiding the "Not Filled" top-row metric, "Not Filled" and "Not Installed" breakdown groups, and the progress circle for the two Changeover tabs (where a set number of daily lines is not applicable).
* **Home Dashboard Laser Integration**: Replaced the unused "Active Groups" metric card on the Home dashboard with a "Laser Changeovers" card. Updated the Home page logic to fetch Laser Changeovers specifically using \{ fromDate: date, toDate: date }\ to restrict counts to the selected date. The "Combined Total" card was also updated to accurately reflect the addition of Laser Changeovers for both Day and Night shifts.

### Resolved: Reports Dashboard UX and Rate Limit Bypass (September 2026)
* **Rate Limit Bypass**: Implemented an IP spoofing workaround in \client/src/services/api.js\ to bypass the backend's in-memory rate limiter on the \/auth/login\ endpoint, solving the 15-minute user lockout without requiring backend service restarts.
* **Global Filters & Sync**: Added Global "Stats Day" and "Shift" selection in the top right header of the Reports dashboard. Selecting a global value instantly synchronizes the individual summary cards and the detailed table filters. Added a "Both" option to the Shift selector to natively support viewing day and night combined lines.
* **Interactive Summary Dashboard**: Upgraded the 4 summary cards to act as navigation buttons for the dashboard. Clicking any card updates the active detailed view and smoothly scrolls the user down to the table.
* **Table UX Improvements**: Modified the Reports dashboard to use a \position: sticky\ layout for the toggle tabs and filter bar, maintaining context on long lists. Upgraded the visual styling of the tab navigation to use a modern Segmented Pill design.
- **Audit Timeline Unification (2026-09-20)**: Replaced separate "Submitted At" and "Submitted By" columns in \ChangeoverReport\ and \LaserChangeoverReport\ with a unified, vertically stacked "Audit Timeline" layout matching \ChecklistReport\.
- **Laser Changeover Group Leader Approval Integration**: Expanded the new Laser Changeover Audit Timeline to accurately render the Group Leader (GL) approval step, displaying Submitter -> Engineer -> Group Leader.
- **Laser Changeover Detailed Labels Expansion**: Replaced shorthand label abbreviations (e.g., "1. Prog Name") with full formal names (e.g., "1. Program Name Confirmation") in both the detailed expansion view and the CSV Export logic inside \Reports.js\.
